...
## References
`~/.claude/skills/tidier/references/` — the 33 chapters verbatim, one file each, plus `java.md`. At step 4 open `NN-<slug>.md` for the tidying you are applying: its exact prompt, move, caveats and chaining. Chapters 16–33 are read differently — each ends in a `## For the tidier` block carrying that chapter's decision rules, and that block is all you take: `awk '/^## For the tidier/{f=1} f' references/21-*.md`, never `Read` the chapter (five times the size; its body only to quote from). Reach for one when the call is genuinely unclear — `21-*` first/after/later/never · `27-*` does it pay · `20-*` dirty tree · `18-*`/`19-*` how much is enough · `17-*` what this opens · `29-*`/`32-*` "touch this and I touch that". Java target: `java.md`, once. A file whose `# ` heading does not name the tidying you looked it up for belongs to another collection numbering its files the same way — treat it as missing. Directory missing or wrong → say so and apply only mechanically safe tidyings.
