...
Read once per run when the target is `.java`. Adds to `catalog.md`; the book's prompt still has to be met exactly.
...
| 8 | Explaining Variables | `final var` (or an explicit type) named for intent. Extracting a subexpression of a `BigDecimal` chain must keep the same operation order and scale. |
